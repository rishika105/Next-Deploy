...
export default function LogsPage() {
  ...
  const fetchDeploymentInfo = async () => {
    try {
      ...
      const response = await axios.get(
        `http://localhost:9000/api/project/deployment/${id}`,
        {
  ...
  const startPollingLogs = async () => {
    try {
      const token = await getToken();
      const response = await axios.get(
        `http://localhost:9000/api/logs/${id}`,
        {
          headers: { Authorization: `Bearer ${token}` },
        }
      );
      ...
      if (response.data.rawLogs) {
        const sortedLogs = response.data.rawLogs.sort((a, b) => 
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="mb-8">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-4xl font-bold mb-2">Build Logs</h1>
              <p className="text-gray-400">
                Real-time logs for deployment: <span className="font-mono text-[#FF9FFC]">{id}</span>
              </p>
            </div>
            {deploymentInfo && (
              <div className="flex items-center space-x-4">
                <div className={`px-4 py-2 rounded-full ${getStatusColor(deploymentInfo.status)}`}>
                ...
                <a
                  ...
                  className="px-4 py-2 border border-[#5227FF] text-[#FF9FFC] hover:bg-[#5227FF]/10 rounded-lg transition-colors"
                >
                  Visit Site
                </a>
              </div>
          ...
          <div className="mt-4 flex items-center">
            <span className={`w-2 h-2 rounded-full mr-2 ${isPolling ? 'bg-green-400 animate-pulse' : 'bg-gray-400'}`}></span>
            <span className="text-sm text-gray-400">
              {isPolling ? 'Live polling active' : 'Polling stopped - deployment completed'}
            </span>
            {!isPolling && (
              <button
                onClick={restartPolling}
                className="ml-4 text-sm text-[#FF9FFC] hover:text-[#5227FF] transition-colors"
              >
                Resume polling
              </button>
            )}
        ...
        <div className="grid lg:grid-cols-4 gap-8">
            ...
            {deploymentInfo && (
              <>
                <div className="bg-gray-900/30 backdrop-blur-sm border border-gray-700 rounded-xl p-6">
                  <h3 className="text-lg font-bold mb-4">Deployment Details</h3>
                  <div className="space-y-3">
                    <div>
                      <p className="text-sm text-gray-400">Status</p>
                      <div className="flex items-center">
                        <p className="font-medium mr-2">{deploymentInfo.status}</p>
                    ...
                    <div>
                      ...
                      <p className="font-medium">{deploymentInfo.projectSlug}</p>
                    </div>
                    <div>
                ...
                <div className="bg-gray-900/30 backdrop-blur-sm border border-gray-700 rounded-xl p-6">
                  ...
                  <div className="space-y-3">
                    <button
                      onClick={refreshLogs}
                      className="w-full bg-[#5227FF] hover:bg-[#5227FF]/80 text-white py-2 rounded-lg transition-colors"
                    >
                      Refresh Now
                    </button>
                    <button
                      onClick={() => setLogs([])}
                      className="w-full border border-gray-700 hover:bg-gray-800/50 text-gray-300 py-2 rounded-lg transition-colors"
                    >
                      Clear Display
                    </button>
                    {isPolling ? (
          ...
          <div className="lg:col-span-3">
            <div className="bg-gray-900/20 backdrop-blur-sm border border-gray-700 rounded-xl overflow-hidden">
              <div className="bg-gray-900/50 border-b border-gray-700 px-6 py-4">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center space-x-4">
                    ...
                    <div className="flex items-center">
                      <span className={`w-2 h-2 rounded-full mr-2 ${isPolling ? 'bg-green-400 animate-pulse' : 'bg-gray-400'}`}></span>
                      <span className="text-sm">{isPolling ? 'Live' : 'Paused'}</span>
                    </div>
                  </div>
                </div>
              </div>

              <div className="h-[600px] overflow-y-auto p-6 font-mono text-sm bg-black/30">
                {loading && logs.length === 0 ? (
                  ...
                  <div className="space-y-1">
                    {logs.map((log, index) => (
                      <div
                        key={index}
                        className={`p-2 rounded ${log.log.includes('Error') || log.log.includes('❌') 
              ...
              {deploymentInfo && FINAL_STATUSES.includes(deploymentInfo.status) && (
                <div className={`px-6 py-3 ${deploymentInfo.status === 'READY' ? 'bg-green-500/10 border-t border-green-500/30' : 'bg-red-500/10 border-t border-red-500/30'}`}>
                  <div className="flex items-center justify-between">
                    <div className="flex items-center">
                    ...
                    <div className="text-sm text-gray-400">
                      Last updated: {new Date(deploymentInfo.updatedAt).toLocaleTimeString()}
                    </div>
                  </div>
                </div>
              )}
